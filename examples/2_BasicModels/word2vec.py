...
# print('update count: %s' % count[0: 10])
...
def next_batch(batch_size, num_skips, skip_window):
    ...
    for i in range(1):
        ...
        print('context_words: %s, words_to_use: %s' % (context_words, words_to_use))

        for j, context_word in enumerate(words_to_use):

            batch[i * num_skips + j] = buffer[skip_window]
            labels[i * num_skips + j, 0] = buffer[context_word]
            print('batch: %s,\n labels: %s' % (batch, labels))
...
# print('batch_x: %s, batch_y: %s' % (batch_x, batch_y))
